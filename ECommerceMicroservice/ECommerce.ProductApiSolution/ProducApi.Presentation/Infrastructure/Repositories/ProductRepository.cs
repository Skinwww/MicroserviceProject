...
namespace ProductApi.Infrastructure.Repositories
{
    public class ProductRepository(ProductDbContext context, IPublishEndpoint publishEndpoint) : IProduct
    {
        ...
        public async Task<Response> DeleteAsync(Product _product)
        {
            try
            {
                var product = await FindByIdAsync(_product.Id);
                if (product is null)
                    return new Response(false, $"{_product.Name} не найдет");
                context.Products.Remove(_product);
                await context.SaveChangesAsync();
                return new Response(true, $"{_product.Name} удален успешно");
            }
            catch (Exception ex)
            {
                LogException.LogExceptions(ex);
