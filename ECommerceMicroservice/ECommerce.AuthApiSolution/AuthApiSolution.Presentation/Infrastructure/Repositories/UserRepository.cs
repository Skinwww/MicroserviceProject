...
{
    public class UserRepository(AuthDbContext context, IConfiguration config, IPublishEndpoint publishEndpoint) : IUserRepository
    {
        ...
        public async Task<Response> Register(AppUserDTO appUserDTO)
        {
            ...
            string content = BuildRegistrationEmailBody();
            await publishEndpoint.Publish(new EmailDTO("Регистрация", content));
            return result.Entity.Id > 0 ? new Response(true, "Регистрация прошла успешно") :
                new Response(false, "Неверные данные");

        }

        private string BuildRegistrationEmailBody()
        {
            var sb = new StringBuilder();
            sb.AppendLine("<h>Благодарим Вас за регистрацию на нашем сайте!</h>");
